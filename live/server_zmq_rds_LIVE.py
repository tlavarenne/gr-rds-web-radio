...
# -----------------------------
# STATIONS (LIVE) — presets
# freq in MHz (matches your GRC variable "freq")
# -----------------------------
STATIONS = [
    {"name": "France Inter",   "freq": 87.8},
    {"name": "France Musique", "freq": 91.7},
    {"name": "France Bleu",    "freq": 107.1},
]
...
STATE = {
    ...
    # freq (MHz) tracked locally for UI
    "freq_mhz": 87.0,
    "last_freq": 0.0,

    # gain tracking (optional, as you had)
    "gain": 0.0,
    "last_gain": 0.0,
}
...
def _set_freq_mhz(mhz, selected_name=None):
    """
    Sets GRC variable "freq" in MHz via XMLRPC: rpc.set_freq(mhz)
    This matches: Qt GUI Range ID = freq
    And RTLSDR center freq = freq*1e6 in your flowgraph.
    """
    mhz = float(mhz)
    if mhz < 87.0: mhz = 87.0
    if mhz > 108.0: mhz = 108.0

    rpc = rpc_client()
    rpc.set_freq(mhz)  # IMPORTANT: your GRC variable must be named "freq"

    with _LOCK:
        STATE["freq_mhz"] = mhz
        STATE["last_freq"] = time.time()
        STATE["selected"] = selected_name  # None if manual
    return mhz
...
def on_audio(d):
    with _LOCK:
        AUDIO["y"] = _limit_list(d.get("y", []) or [], 1400)
...
def on_rds_scope(d):
    with _LOCK:
        RDS_SCOPE["y"] = _limit_list(d.get("y", []) or [], 1400)
...
def on_const(d):
    with _LOCK:
        I = _limit_list(d.get("i", []) or [], 1200)
        Q = _limit_list(d.get("q", []) or [], 1200)
        n = min(len(I), len(Q))
        CONST["i"] = I[:n]
        CONST["q"] = Q[:n]
...
def api_select():
    ...
    try:
        mhz = _set_freq_mhz(target["freq"], selected_name=target["name"])
        return json_nostore({"ok": True, "freq_mhz": mhz})
    except Exception as e:
        return json_nostore({"ok": False, "error": str(e)}), 500

# --------- NEW: Manual frequency slider (MHz) ----------
@app.route("/api/freq", methods=["POST"])
def api_freq():
    data = request.get_json(force=True) or {}
    mhz = data.get("freq_mhz", None)
    try:
        mhz = float(mhz)
    except Exception:
        return json_nostore({"ok": False, "error": "bad freq"}), 400

    try:
        mhz2 = _set_freq_mhz(mhz, selected_name=None)  # manual mode
        return json_nostore({"ok": True, "freq_mhz": mhz2})
    except Exception as e:
        return json_nostore({"ok": False, "error": str(e)}), 500
...
def api_gain():
    data = request.get_json(force=True) or {}
    g = data.get("gain", None)
    try:
        g = float(g)
    except Exception:
        return json_nostore({"ok": False, "error": "bad gain"}), 400
    ...
    try:
        rpc = rpc_client()
        rpc.set_gain(g)  # requires GRC variable named "gain"
        with _LOCK:
            STATE["gain"] = g
            STATE["last_gain"] = time.time()
        return json_nostore({"ok": True, "gain": g})
    except Exception as e:
        return json_nostore({"ok": False, "error": str(e)}), 500
...
.tick{position:absolute;bottom:14px;width:1px;background:rgba(234,240,255,.55);opacity:.75}
...
/* Sliders row */
.row{margin-top:12px; display:flex; align-items:center; gap:12px; flex-wrap:wrap}
.row input[type="range"]{width:320px}
.badge{padding:6px 10px;border-radius:999px;border:1px solid var(--stroke);background:rgba(255,255,255,.06);font-size:12px;color:var(--mut)}
...
<div class="wrap">
  ...
  <div class="topbar">
    <div class="brand">
      <div class="logo"></div>
      <div><div class="title">FM Monitor • LIVE • RDS</div></div>
    </div>
    <div class="chips">
  ...
  <div class="gridTop">
    <div class="card">
      ...
      <!-- NEW: Web frequency slider (MHz) controlling GRC variable freq -->
      <div class="row">
        <div class="badge">Freq: <b id="freqv">87.0</b> MHz</div>
        <input id="freqs" type="range" min="87.0" max="108.0" step="0.1" value="87.0">
        <div class="badge" id="freqmode">sync</div>
      </div>

      <!-- Gain slider (optional) -->
      <div class="row">
        <div class="badge">Gain: <b id="gainv">0</b> dB</div>
        <input id="gain" type="range" min="0" max="45" step="1" value="0">
        <div class="badge" id="gaintxt">sync</div>
      </div>
  ...
  <div class="gridBottom">
    ...
    <div class="card">
      <div class="h">RDS • Constellation</div>
...
let freqLastSent = -999;
let freqCooldownUntil = 0;

// gain slider (web)
const gainEl = document.getElementById("gain");
const gainvEl = document.getElementById("gainv");
const gaintxt = document.getElementById("gaintxt");
let gainLastSent = -999;
let gainCooldownUntil = 0;
  ...
  for(const s of stations){
    ...
    b.onclick = async ()=>{
      ...
      try{
        ...
        // update web slider immediately (visual)
        freqs.value = String(s.freq.toFixed(1));
        freqv.textContent = Number(s.freq).toFixed(1);
        freqmode.textContent = "preset";
...
// --------- send freq (MHz) ----------
async function sendFreq(mhz){
  const now = Date.now();
  if(now < freqCooldownUntil) return;
  freqCooldownUntil = now + 140; // throttle
...
freqs.addEventListener("input", ()=>{
  const f = Number(freqs.value);
  freqv.textContent = f.toFixed(1);
  freqmode.textContent = "manuel";
  selected = null; // manual mode in UI
...
// --------- send gain ----------
async function sendGain(g){
  const now = Date.now();
  if(now < gainCooldownUntil) return;
  gainCooldownUntil = now + 160; // throttle
  try{
...
gainEl.addEventListener("input", ()=>{
  ...
  gainvEl.textContent = g.toFixed(0);
  gaintxt.textContent = "manuel";
  if(g !== gainLastSent){
    gainLastSent = g;
    sendGain(g);
  }
...
async function tick(){
  ...
  // selection label
  selected = (st.selected !== undefined && st.selected !== null) ? st.selected : selected;
  document.getElementById("sel").textContent = selected || "—";
  document.getElementById("preset").textContent = selected || "manuel";

  // freq sync (from server state)
  const f = Number(st.freq_mhz || 0);
  if(Number.isFinite(f) && f > 0){
    // if user not dragging web slider, keep in sync
    if(document.activeElement !== freqs){
      freqs.value = String(f.toFixed(1));
      freqv.textContent = f.toFixed(1);
      freqmode.textContent = selected ? "preset" : "sync";
    }
    setCursorFreq(f);
  }
  ...
  // gain sync (if not dragging)
  const g = Number(st.gain || 0);
  if(Number.isFinite(g)){
    if(document.activeElement !== gainEl){
      gainEl.value = String(Math.round(g));
      gainvEl.textContent = g.toFixed(0);
      gaintxt.textContent = "sync";
    }
  }
...
window.addEventListener("resize", ()=>{
  ...
  buildDial();
  const f = Number(freqs.value);
  if(Number.isFinite(f)) setCursorFreq(f);
});
</script>
</body>
</html>
...
if __name__ == "__main__":
    ...
    # Optional: initialize freq to 87.0 MHz at startup
    try:
        _set_freq_mhz(87.0, selected_name=None)
    except Exception:
        pass
